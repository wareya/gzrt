...
/* Fix CRC */
static void fix_crc ( void )
{
	/* ROM file chosen? */
	if( !rom_name )
	{
		msg( "Error", "You need to choose a ROM filename first." );
		return;
	}
}
...
void create_window ( void )
{
	...
	button = gtk_button_new_with_label( "CRC" );
	g_signal_connect_swapped( G_OBJECT(button), "clicked", G_CALLBACK(fix_crc), NULL );
	gtk_box_pack_start( GTK_BOX(hbox), button, FALSE, TRUE, 0 );

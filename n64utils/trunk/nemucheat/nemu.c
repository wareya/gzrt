...
nc_free ( void )
{
    int i, l = g_list_length( games ), L, j;
    int cheatcount;
    struct Game * g;
    struct Cheat * C;
    ...
    for( i = 0; i < l; i++ )
    {
        g = g_list_nth( games, i )->data;
        
        cheatcount = g_list_length( g->cheats );
        
        for( j = 0; j < cheatcount; j++ )
        {
            C = g_list_nth( g->cheats, j )->data;
            
            free( C->codes );
            free( C );
        }
        
        g_list_free( g->cheats );
        free( g );
    }
...
nc_rebuild_ini ( void )
{
    int i, k, j;
    int cheattotal;
    int gametotal = g_list_length( games );
    struct Game * cur;
    struct Cheat * cheat;
    FILE * out;
    ...
    {
        cur = g_list_nth( games, i )->data;
        cheattotal = g_list_length( cur->cheats );
        
        /* Write game title */
        fprintf( out, "[%s]\n", cur->name );
        
        /* Write number of cheats */
        fprintf( out, "NumCheats=%i\n", cheattotal );
        
        /* Loop through the cheats */
        for( k = 0; k < cheattotal; k++ )
        {
            cheat = g_list_nth( cur->cheats, k )->data;
            
            /* Write title & count */
            fprintf( out, "CheatName%i=%s\n", k, cheat->name );
            fprintf( out, "CheatName%iCount=%i\n", k, cheat->count );
            
            /* Write codes */
            for( j = 0; j < cheat->count; j++ )
            {
                fprintf( out, "CheatName%iCode%i=%08X %04X\n", k, j, 
                cheat->codes[j].address, cheat->codes[j].value );
            }
        }

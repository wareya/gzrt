...
int crc_rom ( char * filename )
{
    ...
    buff = malloc(CHECKSUM_LENGTH + CHECKSUM_START);
    fseek( handle, 0, SEEK_SET );
    fread( buff, 1, CHECKSUM_LENGTH + CHECKSUM_START, handle );
    ...
    fseek( handle, N64_CRC1, SEEK_SET );
    
    for( i = 0; i < 2; i++ )
        for( k = 0; k < 4; k++ )
            fputc( CRC[i] >> (24 - k * 8), handle );
